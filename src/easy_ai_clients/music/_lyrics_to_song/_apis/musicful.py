...
ENDPOINT = "https://api.musicful.ai/v1/music/generate"
STATUS_ENDPOINT = "https://api.musicful.ai/v1/music/tasks"


def generate_lyrics_to_song(lyrics, prompt=None, output_path=None, sync=True, **kwargs):
    """Generate a song from lyrics with Musicful.
    ...
    Args:
        ...
        **kwargs: Optional. Musicful fields.
...
def get_generation_status(request_id, **kwargs):
    """Return Musicful task status.

    Args:
        request_id: Required. Musicful task ID.
...
def _build_payload(lyrics, prompt=None, model=None, **kwargs):
    """Build a Musicful payload.

    Args:
        lyrics: Required. Lyrics or structured song text.
        prompt: Optional. Style prompt.
        model: Optional. Musicful model version.
        **kwargs: Optional. Musicful fields.
    ...
    if instrumental in (1, True, "1", "true"):
        raise ValueError("Musicful instrumental generation should not include lyrics.")
    action = provider_api.pop_value(kwargs, "action", default="custom")
    style = provider_api.pop_value(kwargs, "style", default=prompt)
    payload = {"action": action, "mv": model or DEFAULT_MODEL, "lyrics": lyrics}
    provider_api.add_optional(payload, style=style)
    return provider_api.merge_kwargs(payload, kwargs)


def _headers():
    """Return Musicful headers.
