...
VOICE_ENDPOINT_PATH = "/v5/generate/song"
...
def convert_voice(audio, voice=None, prompt=None, output_path=None, sync=True, **kwargs):
    """Run a Soundverse singing or voice conversion style flow.

    Args:
        audio: Required. Vocal URL, instrumental URL, or source music audio.
        voice: Optional. Voice ID, singer, or reference voice value.
        prompt: Optional. Style or singing instruction.
        output_path: Optional. Destination path for a final URL.
        sync: Optional. Poll status when a message ID is returned.
    ...
    return _ops.result(
        ...
        metadata=_ops.provider_metadata(
            raw_response,
            audio,
            extra={"voice": voice, "conversion_flow": "ai_singing_or_song"},
        ),
...
def get_generation_status(request_id, **kwargs):
    """Get Soundverse voice task status.
...
def get_generation_result(request_id, output_path=None, **kwargs):
    """Get Soundverse voice task result.
...
def download_generation(request_id, output_path=None, **kwargs):
    """Download a completed Soundverse voice result.
...
def _build_payload(audio, voice=None, prompt=None, **kwargs):
    """Build the Soundverse voice conversion payload.

    Args:
        audio: Required. Vocal or instrumental audio.
        voice: Optional. Voice ID or reference voice.
    ...
    _ops.add_audio_input(payload, audio, url_key="vocal_url", generic_key="vocal_url")
    return payload
...
def _cost(raw_response=None, payload=None):
    ...
    return _ops.unavailable_cost(
        {"reason": "Soundverse public docs do not expose voice conversion cost."}
    )
