...
def generate_video_to_video(prompt=None, video_path=None, video_url=None, output_path=None, sync=True, **kwargs):
    model = kwargs.pop("model", DEFAULT_MODEL)
    body = common.payload(model, prompt, kwargs)
    video = common.media_object(video_path, video_url, "video_path", "video_url")
    if not video:
        raise ValueError("xAI video_to_video requires video_path or video_url.")
    body["video"] = video
...
__all__ = ["DEFAULT_MODEL", "download_generation", "generate_video_to_video", "get_generation_result", "get_generation_status"]
