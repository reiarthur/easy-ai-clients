"""Google Veo image-to-video wrapper for the Gemini Developer API."""
...
def _selected_model(kwargs):
    ...
    if model not in MODEL_DATA:
        raise ValueError(f"Unsupported Google Veo image-to-video model: {model}.")
    return model


def _inline_image(mime_type, payload):
    return {"inlineData": {"mimeType": mime_type or "image/png", "data": payload}}


def _remote_image_object(url, timeout_seconds=None):
    ...
    try:
        ...
        raise RuntimeError(f"Image download failed with HTTP {exc.code}: {message}") from exc
    except urllib.error.URLError as exc:
        raise RuntimeError(f"Image download failed for {url}: {exc.reason}") from exc
    mime_type = content_type or mimetypes.guess_type(str(url))[0] or "image/png"
    payload = base64.b64encode(body).decode("ascii")
    return _inline_image(mime_type, payload)


def _image_object(value, timeout_seconds=None):
    ...
    if text.lower().startswith(("http://", "https://")):
        return _remote_image_object(text, timeout_seconds=timeout_seconds)
    return {"uri": text}
...
def _build_payload(model, prepared, kwargs):
    ...
    instance = {
        ...
        "image": _image_object(prepared["image"], timeout_seconds=kwargs.get("timeout_seconds")),
    }
    if kwargs.get("last_image_url"):
        instance["lastFrame"] = _image_object(
            kwargs.get("last_image_url"),
