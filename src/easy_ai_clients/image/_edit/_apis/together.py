...
from ..._common.together_utils import edit_image as _edit_image
from ..post_processing import build_edit_result
from ..pre_processing import prepare_edit_inputs

_DEFAULTS = {'mask': None, 'width': 1024, 'height': 1024, 'output_format': 'png', 'seed': None, 'timeout_seconds': 120}
_EXTRA_KEYS = ('negative_prompt', 'guidance_scale', 'num_images', 'image_loras', 'style', 'scheduler')


def edit(prompt: str, image: str, model: str = 'black-forest-labs/FLUX.2-dev', **kwargs):
    """Edit an image with Together AI and return the normalized public contract.
...
validation details are documented in edit/docs/together_ai_api.md."""
    ...
    try:
        prepared = prepare_edit_inputs(prompt, image, provider='together_ai', mask=values["mask"])
        return _edit_image(
