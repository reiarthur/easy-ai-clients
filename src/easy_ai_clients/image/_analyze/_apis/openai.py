"""OpenAI analyze API wrapper."""

from __future__ import annotations

from ..._common.env_utils import get_provider_api_key
from ..._common.openai_utils import analyze_image as _analyze_image
...
_DEFAULTS = {'service_tier': '', 'reasoning': '', 'timeout_seconds': 60}
_EXTRA_KEYS = ('max_output_tokens', 'temperature', 'top_p', 'text', 'metadata', 'user', 'instructions', 'store', 'parallel_tool_calls', 'tools', 'tool_choice', 'response_format')


def analyze(prompt: str, image: str, model: str = 'gpt-4.1-nano', **kwargs):
    """Analyze an image with OpenAI and return the normalized public contract.
...
validation details are documented in analyze/docs/openai_api.md."""
    ...
    extra_body = payload_from_keys(values, ('max_output_tokens', 'temperature', 'top_p', 'text', 'metadata', 'user', 'instructions', 'store', 'parallel_tool_calls', 'tools', 'tool_choice', 'response_format'))
