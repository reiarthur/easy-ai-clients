...
from ..._common.xai_utils import analyze_image as _analyze_image
from ..post_processing import build_analyze_result
from ..pre_processing import prepare_analyze_inputs

_DEFAULTS = {'reasoning': 'low', 'timeout_seconds': 60}
_EXTRA_KEYS = ('max_output_tokens', 'temperature', 'top_p', 'metadata', 'user', 'instructions', 'store', 'tools', 'tool_choice', 'response_format')


def analyze(prompt: str, image: str, model: str = 'grok-4-fast-non-reasoning', **kwargs):
    """Analyze an image with xAI and return the normalized public contract.
...
validation details are documented in analyze/docs/xai_api.md."""
    ...
    extra_body = payload_from_keys(values, ('max_output_tokens', 'temperature', 'top_p', 'metadata', 'user', 'instructions', 'store', 'tools', 'tool_choice', 'response_format'))
