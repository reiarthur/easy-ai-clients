# ElevenLabs Music Media To Music API

Implementation status: implemented

## Overview

- Operation name: `media_to_music`.
- Provider identifier: `elevenlabs`.
- Credential environment variable: `ELEVENLABS_API_KEY`.
- Supported technical capabilities: music generation guided by image, video, or visual media. Provider coverage includes structured composition, vocal songs, video-to-music, and stem separation.
...
- Endpoint or task flow: `ENDPOINT` = `https://api.elevenlabs.io/v1/music/video-to-music`

## Parameter Reference

```python
easy_ai_clients.music.media_to_music(media, prompt=None, model=None, *, api, **kwargs)
```

Required inputs:

- `media`, `api`, and the provider credential environment variable.
- Provider-native required input notes: `prompt`, `composition_plan`, section lines, video media, and source audio for stems.

Optional inputs:

- `prompt`, `model`; provider-native fields such as media MIME type, style, duration, format, webhook, and endpoint controls.
...
- Endpoint or task flow: REST composition, video-to-music, or stem-separation flow.
...
- Result and download behavior: audio in the selected format, or ZIP output for stems. If `output_path` is supplied, the wrapper attempts to save returned audio when a final URL, bytes, base64, data URI, or compatible provider object is present.
...
- Limitations or warnings: Stem outputs can be ZIP files or grouped artifacts, not a single audio track. Verify rights and privacy for supplied media.
...
result = music.media_to_music(
    "https://example.com/scene.jpg",
    prompt="Tense cinematic underscore.",
    api="elevenlabs",
    output_path="out/score.mp3",
)
```
