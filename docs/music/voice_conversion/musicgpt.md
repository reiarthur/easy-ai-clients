...
- Supported technical capabilities: conversion or application of a vocal identity in a musical workflow. Provider coverage includes prompt-to-song, lyrics-to-song, auto-lyrics, remix, extension, inpainting, image-to-song, cover, and voice changer workflows.
...
- Endpoint or task flow: `DEFAULT_ENDPOINT` = `https://api.musicgpt.com/api/public/v1/MusicAI`; status/result lookup uses `https://api.musicgpt.com/api/public/v1/byId` with `conversionType=MUSIC_AI` and `task_id`.
...
- Limitations or warnings: Separate MusicGPT task endpoints by use case to avoid mixed payload contracts. Voice consent, likeness rights, and allowed-use policy checks are the caller responsibility.
