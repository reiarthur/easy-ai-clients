# fal.ai Music Models Music Edit API
...
- Supported technical capabilities: continuation, extension, inpainting, repainting, or repair of existing music. Provider coverage includes music generation, lyrics-to-song, audio-to-audio, remix, inpaint, outpaint, clips, and samples through hosted model pages.
...
- Endpoint or task flow: fal queue, subscribe, Python client, JavaScript client, or raw HTTP depending on model. This wrapper path may need explicit endpoint or base URL kwargs before a request can be assembled.
