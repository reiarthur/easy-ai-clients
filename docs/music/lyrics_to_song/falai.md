# fal.ai Music Models Lyrics To Song API
...
- Supported technical capabilities: song generation where lyrics are the primary structured input. Provider coverage includes music generation, lyrics-to-song, audio-to-audio, remix, inpaint, outpaint, clips, and samples through hosted model pages.
...
- Endpoint or task flow: `ENDPOINT` = `https://queue.fal.run/{model}`; `STATUS_ENDPOINT` = `https://queue.fal.run/{model}/requests/{request_id}/status`; `RESULT_ENDPOINT` = `https://queue.fal.run/{model}/requests/{request_id}/response`
