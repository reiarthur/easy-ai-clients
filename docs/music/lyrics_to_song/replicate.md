# Replicate Hosted Music Models Lyrics To Song API
...
- Supported technical capabilities: song generation where lyrics are the primary structured input. Provider coverage includes Python-first hosted music models for text, lyrics, vocals, audio guidance, continuation, and loops.
...
- Endpoint or task flow: `STATUS_ENDPOINT` = `https://api.replicate.com/v1/predictions/{request_id}`; `PREDICTIONS_ENDPOINT` = `https://api.replicate.com/v1/predictions`; `MODEL_ENDPOINT` = `https://api.replicate.com/v1/models/{model}/predictions`
