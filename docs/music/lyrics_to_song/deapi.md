# deAPI Text-to-Music Lyrics To Song API
...
- Supported technical capabilities: song generation where lyrics are the primary structured input. Provider coverage includes vocal music, instrumental generation, lyrics-to-song, parameter control, audio reference, audio-to-audio, and remix variation.
...
- Endpoint or task flow: `ENDPOINT` = `https://api.deapi.ai/api/v2/audio/music`; `STATUS_ENDPOINT` = `https://api.deapi.ai/api/v2/jobs/{request_id}`
