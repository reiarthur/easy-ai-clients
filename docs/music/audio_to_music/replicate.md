# Replicate Hosted Music Models Audio To Music API
...
- Supported technical capabilities: music generation or transformation guided by source audio. Provider coverage includes Python-first hosted music models for text, lyrics, vocals, audio guidance, continuation, and loops.
...
- Endpoint or task flow: `DEFAULT_ENDPOINT` = `https://api.replicate.com/v1/predictions`
...
- Limitations or warnings: Use only models whose terms and model page match the music use case. Verify rights to the source audio before transformation or editing.
