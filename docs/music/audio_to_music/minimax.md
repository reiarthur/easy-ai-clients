# MiniMax Music Audio To Music API
...
- Supported technical capabilities: music generation or transformation guided by source audio. Provider coverage includes vocal song generation, auto-lyrics, instrumental generation, and cover or re-style from reference audio.
...
- Default model: `music-cover`.
- Endpoint or task flow: `DEFAULT_ENDPOINT` = `https://api.minimax.io/v1/music_generation`
...
Current wrapper default: `music-cover`.

Pass `model=...` to override when the adapter and provider endpoint support model selection.

## Domain Notes

- Endpoint or task flow: MiniMax REST music generation and related cover/lyrics endpoints.
...
- Limitations or warnings: Do not combine incompatible lyrics, instrumental, and lyrics optimizer modes. Verify rights to the source audio before transformation or editing.
