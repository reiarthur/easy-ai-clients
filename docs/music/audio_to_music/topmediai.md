# TopMediai AI Music Generator Audio To Music API
...
- Supported technical capabilities: music generation or transformation guided by source audio. Provider coverage includes vocal music, instrumental music, lyrics-to-song, audio reference generation, extension, singer generation, and MP4 or WAV export.
...
- Default model: `v4.5`.
- Endpoint or task flow: `DEFAULT_ENDPOINT` = `https://api.topmediai.com/v3/music/generate`; `DEFAULT_BASE_URL` = `https://api.topmediai.com`
...
Current wrapper default: `v4.5`.

Pass `model=...` to override when the adapter and provider endpoint support model selection.

## Domain Notes

- Endpoint or task flow: REST task flow with generation, task query, singer generation, and format conversion endpoints.
...
- Result and download behavior: task lookup uses `ids`; completed tasks may expose MP3, MP4, or WAV URLs depending on the endpoint. If `output_path` is supplied, the wrapper attempts to save returned audio when a final URL, bytes, base64, data URI, or compatible provider object is present.
...
- Limitations or warnings: Some wrappers need `base_url`, `endpoint`, or status endpoint controls when only endpoint paths are present locally. Verify rights to the source audio before transformation or editing.
