# Scenario Meta MusicGen Audio To Music API
...
- Supported technical capabilities: music generation or transformation guided by source audio. Provider coverage includes short instrumental clips, loops, samples, assets, and melody-guided generation through Meta MusicGen.
...
- Default model: `stereo-melody-large`.
- Endpoint or task flow: `DEFAULT_ENDPOINT` = `https://api.cloud.scenario.com/v1/generate/custom/{model_id}`
...
Current wrapper default: `stereo-melody-large`.
...
- Limitations or warnings: This provider is better for short assets than long complete songs. Verify rights to the source audio before transformation or editing.
