# fal.ai Music Models Audio To Music API
...
- Supported technical capabilities: music generation or transformation guided by source audio. Provider coverage includes music generation, lyrics-to-song, audio-to-audio, remix, inpaint, outpaint, clips, and samples through hosted model pages.
...
- Endpoint or task flow: `DEFAULT_QUEUE_BASE_URL` = `https://queue.fal.run`
...
- Cost behavior: Use the fal model pricing page. The MiniMax music v2.6 page lists `0.15` per generated audio.
- Persistence notes: CDN retention is not confirmed for all models locally. Download returned files promptly.
- Limitations or warnings: Treat every fal model page as a separate payload, price, and license contract. Verify rights to the source audio before transformation or editing.
...
Use the fal model pricing page. The MiniMax music v2.6 page lists `0.15` per generated audio.
