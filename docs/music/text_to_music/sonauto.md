# Sonauto Melodia Text To Music API
...
- Supported technical capabilities: prompt-led music generation from a text brief. Provider coverage includes full song, instrumental, lyrics, duration control, audio-guided music, extension, inpainting, streaming, and webhook workflows.
...
- Default model: `v3`.
- Endpoint or task flow: `BASE_URL` = `https://api.sonauto.ai/v1`; `GENERATE_PATH` = `/generations/v3`; `STATUS_PATH` = `/generations/status/{task_id}`; additional status/result path constants are defined in the adapter
