# Cloudflare Workers AI MiniMax Music Text To Music API
...
- Supported technical capabilities: prompt-led music generation from a text brief. Provider coverage includes Workers AI hosted MiniMax Music for vocal or instrumental generation, lyrics-to-song, and prompt-to-vocals.
...
- Endpoint or task flow: `POST https://api.cloudflare.com/client/v4/accounts/{account_id}/ai/run` with body containing `model` and `input`.
