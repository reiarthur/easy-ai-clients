...
```python
from text.apis import deepinfra

result = deepinfra.generate("Reply with OK.")
...
```python
result = deepinfra.generate("Reply with OK.", model="Qwen/Qwen3.5-0.8B", max_tokens=64, temperature=0, top_p=1, stop=["END"], stream=True)
```

## J. Pricing section
- Pricing snapshot date: `2026-04-25`.
- Pricing source: see the official references above and the validation artifact `validacao_text_apis_2026-04-25/catalog_deepinfra.json`.
...
- Provider result counts in the final matrix: `{'passed': 85, 'failed': 6}`.
- Failure blocker counts: `{'provider_internal_error': 1, 'provider_rate_limit_or_overload': 1, 'catalog_listed_but_not_callable_in_endpoint': 3, 'provider_timeout': 1}`.
