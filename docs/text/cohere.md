...
```python
from text.apis import cohere

result = cohere.generate("Reply with OK.")
print(result["output_text"])
```

### Explicit instruction
```python
result = cohere.generate(
...
```python
result = cohere.generate("Reply with OK.", max_tokens=64, temperature=0, p=1, k=0)
```

## J. Pricing section
- Pricing snapshot date: `2026-04-25`.
- Pricing source: see the official references above and the validation artifact `validacao_text_apis_2026-04-25/catalog_cohere.json`.
