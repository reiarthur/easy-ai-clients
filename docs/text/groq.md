# Groq Text API Wrapper

## A. Overview
- The wrapper targets the OpenAI-compatible Groq chat completions API.
- Raw generation endpoint: `POST https://api.groq.com/openai/v1/chat/completions`.
- Live catalog endpoint: `GET https://api.groq.com/openai/v1/models`.
...
- Create account / console: https://console.groq.com/
- API key docs: https://console.groq.com/keys
...
- Public function signature: `generate(input_text, instruction=None, model='llama-3.1-8b-instant', **kwargs)`.
- Current default model: `llama-3.1-8b-instant`.
...
- Tie-break reasoning: llama-3.1-8b-instant is the lowest-cost general chat model validated in Groq's model/pricing surface.
...
- Model restrictions: validated for `groq` at family/model level in the validation matrix; unsupported combinations fail provider-side and are recorded as blockers.
...
- Relevant text/chat model count after scope filtering: `9` from raw catalog count `16`.
...
|---|---|---|---|
| llama-3.1-8b-instant | llama | passed | validated |
| qwen/qwen3-32b | qwen | passed | validated |
| groq/compound | groq | passed | validated |
| groq/compound-mini | groq | passed | validated |
| meta-llama/llama-4-scout-17b-16e-instruct | meta-llama | passed | validated |
| openai/gpt-oss-20b | openai | passed | validated |
| llama-3.3-70b-versatile | llama | passed | validated |
| allam-2-7b | allam | passed | validated |
...
```python
from text.apis import groq

result = groq.generate("Reply with OK.")
...
```python
result = groq.generate("Reply with OK.", model="llama-3.1-8b-instant", max_tokens=64, temperature=0, top_p=1, stop=["END"], stream=True)
```

## J. Pricing section
- Pricing snapshot date: `2026-04-25`.
- Pricing source: see the official references above and the validation artifact `validacao_text_apis_2026-04-25/catalog_groq.json`.
...
- Provider result counts in the final matrix: `{'passed': 14}`.
