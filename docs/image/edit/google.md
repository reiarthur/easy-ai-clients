# Google Gemini Edit API

## Overview

This module implements prompt-guided image editing for Google Gemini using the repository-wide normalized public contract. The public function is `edit(prompt, image, model="<lowest_cost_default>", **kwargs)` and returns `cust_usd`, `base64`, `warnings`, `request_id`.
...
```python
from edit.apis import google_gemini_api

result = google_gemini_api.edit(
