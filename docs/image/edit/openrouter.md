...
This module implements prompt-guided image editing for OpenRouter using the repository-wide normalized public contract. The public function is `edit(prompt, image, model="<lowest_cost_default>", **kwargs)` and returns `cust_usd`, `base64`, `warnings`, `request_id`.
...
```python
from edit.apis import openrouter_api

result = openrouter_api.edit(
