# Black Forest Labs Remix API

## Overview

This module implements reference-conditioned image generation for Black Forest Labs using the repository-wide normalized public contract. The public function is `remix(prompt, reference_images, **kwargs)` and returns `cust_usd`, `base64`, `warnings`, `request_id`.
...
For `remix`, the public signature intentionally has no dedicated `model` parameter. When no `model` keyword is supplied, `flux-2-klein-4b` is used internally as the cheapest relevant reference-capable default currently implemented for this provider.
...
```python
from remix.apis import black_forest_labs_api

result = black_forest_labs_api.remix(
