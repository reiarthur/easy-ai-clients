# Stability AI Remix API

## Overview

This module implements reference-conditioned image generation for Stability AI using the repository-wide normalized public contract. The public function is `remix(prompt, reference_images, **kwargs)` and returns `cust_usd`, `base64`, `warnings`, `request_id`.
...
- Default model: `stable-image-style`
...
For `remix`, the public signature intentionally has no dedicated `model` parameter. When no `model` keyword is supplied, `stable-image-style` is used internally as the cheapest relevant reference-capable default currently implemented for this provider.
...
```python
from remix.apis import stability_ai_api

result = stability_ai_api.remix(
