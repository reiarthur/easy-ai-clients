# Together AI Remix API

## Overview

This module implements reference-conditioned image generation for Together AI using the repository-wide normalized public contract. The public function is `remix(prompt, reference_images, **kwargs)` and returns `cust_usd`, `base64`, `warnings`, `request_id`.
...
For `remix`, the public signature intentionally has no dedicated `model` parameter. When no `model` keyword is supplied, `black-forest-labs/FLUX.2-dev` is used internally as the cheapest relevant reference-capable default currently implemented for this provider.
...
```python
from remix.apis import together_ai_api

result = together_ai_api.remix(
