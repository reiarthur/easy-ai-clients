...
This module implements image-to-text analysis for Google Gemini using the repository-wide normalized public contract. The public function is `analyze(prompt, image, model="<lowest_cost_default>", **kwargs)` and returns `request_id`, `cost_usd`, `input_text`, `output`.
...
```python
from analyze.apis import google_gemini_api

result = google_gemini_api.analyze(
