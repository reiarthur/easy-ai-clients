...
This module implements image-to-text analysis for OpenRouter using the repository-wide normalized public contract. The public function is `analyze(prompt, image, model="<lowest_cost_default>", **kwargs)` and returns `request_id`, `cost_usd`, `input_text`, `output`.
...
```python
from analyze.apis import openrouter_api

result = openrouter_api.analyze(
