# Together AI Analyze API

## Overview

This module implements image-to-text analysis for Together AI using the repository-wide normalized public contract. The public function is `analyze(prompt, image, model="<lowest_cost_default>", **kwargs)` and returns `request_id`, `cost_usd`, `input_text`, `output`.
...
```python
from analyze.apis import together_ai_api

result = together_ai_api.analyze(
