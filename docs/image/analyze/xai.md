...
This module implements image-to-text analysis for xAI using the repository-wide normalized public contract. The public function is `analyze(prompt, image, model="<lowest_cost_default>", **kwargs)` and returns `request_id`, `cost_usd`, `input_text`, `output`.
...
- Default model: `grok-4-fast-non-reasoning`
...
```python
from analyze.apis import xai_api

result = xai_api.analyze(
