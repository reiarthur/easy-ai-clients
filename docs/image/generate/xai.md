...
This module implements text-to-image generation for xAI using the repository-wide normalized public contract. The public function is `generate(prompt, model=None, **kwargs)` and returns `cust_usd`, `base64`, `warnings`, `request_id`.
