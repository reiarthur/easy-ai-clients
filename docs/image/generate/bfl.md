...
This module implements text-to-image generation for Black Forest Labs using the repository-wide normalized public contract. The public function is `generate(prompt, model="<lowest_cost_default>", **kwargs)` and returns `cust_usd`, `base64`, `warnings`, `request_id`.
...
```python
from generate.apis import black_forest_labs_api

result = black_forest_labs_api.generate(
