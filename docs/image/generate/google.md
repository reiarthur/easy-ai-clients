# Google Gemini Generate API

## Overview

This module implements text-to-image generation for Google Gemini using the repository-wide normalized public contract. The public function is `generate(prompt, model=None, **kwargs)` and returns `cust_usd`, `base64`, `warnings`, `request_id`.
